export function validate (name: string, value: string, aux: string = ''): string {
    ...
    if (name === 'login') {
        ...
        if (/^[A-ZА-Я][a-zA-Z\-]*$/.test(value) === false) {
            result = `Требования к полю Имя: латиница или кириллица, первая букава заглавная, без цифр и спец символов (дефис допустим)`;
        }
    } else if (name === 'second_name') {
        if (/^[A-ZА-Я][a-zA-Z\-]*$/.test(value) === false) {
            result = `Требования к полю Фамилия: латиница или кириллица, первая букава заглавная, без цифр и спец символов (дефис допустим)`;
        }
    }
